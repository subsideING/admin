...
3、如果有选择第三级权限，那么一级和二级权限也必须勾选
  ...
  <?php if(is_array($datab)): foreach($datab as $key=>$vo): ?><tr>
	...
	<label>
	<input <?php if(strstr($admin_group['rules'],$vo['id']) == true): ?>checked<?php endif; ?> name="new_rules[]" class="ace ace-checkbox-2" type="checkbox"  value="<?php echo ($vo["id"]); ?>" /><span class="lbl"> <strong><?php echo ($vo["title"]); ?></strong></span>	</label></td>
  </tr>
  <?php if(is_array($vo['sub'])): foreach($vo['sub'] as $key=>$sub): ?><tr>
	...
	<label>
	<input <?php if( strpos($admin_group['rules'],','.$sub['id'].',') > -1 ): ?>checked<?php endif; ?> name="new_rules[]" id="<?php echo ($sub["id"]); ?>" class="ace ace-checkbox-2" type="checkbox"  value="<?php echo ($sub["id"]); ?>" /><span class="lbl"> <?php echo ($sub["title"]); ?></span></label></td>
  </tr>
  <tr>
    ...
    <input <?php if( strpos($admin_group['rules'],','.$subb['id'].',') > -1 ): ?>checked<?php endif; ?> name="new_rules[]" class="ace ace-checkbox-2" type="checkbox"  id="<?php echo ($subb["id"]); ?>" value="<?php echo ($subb["id"]); ?>" />
